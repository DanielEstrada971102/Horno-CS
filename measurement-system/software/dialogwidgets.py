...
def resource_path(relative_path):
    """Get absolute path to resource, work for dev and for pyinstaller"""
    try:
...
user_ui_filename = Path(resource_path("ui_files/user_dialog.ui"))
info_ui_filename = Path(resource_path("ui_files/info_dialog.ui"))
settings_ui_filename = Path(resource_path("ui_files/settings_dialog.ui"))
streaming_ui_filename = Path(resource_path("ui_files/streaming_dialog.ui"))
    ...
    def update_param(self, index):
        if isinstance(self.sender(), QtWidgets.QSpinBox):
            ...
            new_value = QFileDialog.getExistingDirectory(
                self, "Output path",
                resource_path(self.default_params.get("out_path", "./results"))
            )
